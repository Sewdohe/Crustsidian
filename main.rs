...
fn parse_task_file(path: &Path) -> Result<Task> {
    ...
    let frontmatter = extract_frontmatter(&content).context("No frontmatter found")?;
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Today => {
            let today_tasks: Vec<_> = tasks.iter().filter(|t| t.is_due_today()).collect();
            let json = serde_json::to_string_pretty(&today_tasks)?;
            println!("{}", json);
        }
        Commands::Overdue => {
            let overdue_tasks: Vec<_> = tasks.iter().filter(|t| t.is_overdue()).collect();
            let json = serde_json::to_string_pretty(&overdue_tasks)?;
            println!("{}", json);
        }
        Commands::Pending => {
            let pending_tasks: Vec<_> = tasks.iter().filter(|t| !t.is_done()).collect();
            let json = serde_json::to_string_pretty(&pending_tasks)?;
            println!("{}", json);
        }
        Commands::CompletedToday => {
            let completed_today: Vec<_> = tasks.iter().filter(|t| t.is_completed_today()).collect();
